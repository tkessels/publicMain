 ...
 * 
 * Diese Klasse stellt einen Tab für ein ChatWindow bereit und enthält ein Icon
 * für Privat bzw. Gruppenchat, einen Titel für den Chat und ein Icon um den Tab
 * zu schließen. Desweiteren wird dafür gesorgt das bei inaktivem Tab mit
 * ungelesener Nachricht der Titel blinkt. Außerdem wird ein Tab zu einem
 * Offlinechatpartner ausgegraut.
 * 
 * @author ATRM
 * 
 */
public class ChatWindowTab extends JPanel {
	 ...
	 * 
	 * @param title
	 *            Der Titel des ChatWindowTabs.
	 * @param parent
	 *            Das Eltern-JTabbedPane.
	 * @param owner
	 *            Das ChatWindow zu dem der ChatWindowTab gehören soll.
	 */
	ChatWindowTab(String title, JTabbedPane parent, ChatWindow owner) {
		...
		// Das Icon für den Tab erzeugen und je nach ChatTyp (privat/gruppe)
		// setzen:
		...
		// Das Icon (lblIcon), das TitelLabel (lblTitle) + SchließenLabel
		// (btnClose) zum Tab (pnlTab) hinzufügen:
		this.add(lblIcon);
		this.add(lblTitle);
		this.add(lblClose);
	}// eom ChatWindowTab()
	 ...
	 * 
	 * Diese Methode wird verwendet um bei einem Aliaswechsel die Beschriftung
	 * des ChatWindowTabs zu aktualisieren.
	 */
	void updateAlias() {
	...
	void setOffline() {
		...
		this.tabCloseImgIcon.setImage(Help.getIcon("TabCloseGray.png", 10)
				.getImage());
		this.lblIcon.setIcon(Help.getIcon("privateOffline.png"));
	}// eom setOffline()
	 ...
	 * 
	 * Diese Methode sorgt dafür das die Elemente des ChatWindowTabs farbig
	 * werden wenn ein zuvor offline gegangener Chatpartner in einem privaten
	 * Chat wieder online kommt.
	 */
	void setOnline() {
		this.lblTitle.setForeground(Color.BLACK);
		this.tabCloseImgIcon.setImage(Help.getIcon("TabCloseBlack.png", 10)
				.getImage());
		this.lblIcon.setIcon(Help.getIcon("private.png"));
	}// eom setOnline()
	 ...
	 * Nachricht empfangen wird und dieses ChatWindow nicht aktiv ist. Ist der
	 * Tabtitel schwarz wird er orange gesetzt und umgekehrt. Wenn das
	 * ChatWindow selectiert wird, wird das blinken gestoppt.
	 */
	private void blink() {
	...
	/**
	 * Diese Methode stoppt den Thread der für das Blinken zuständig ist und
	 * setzt den Tabtitel auf schwarz.
	 */
	void stopBlink() {
		if (blinker != null) {
			blinker.stopit();
	...
	/**
	 * Diese Elementklasse stellt einen MouseAdapter für das ChatWindowTab
	 * bereit
	 * 
	 * @author ATRM
	 * 
	...
	private class MyMouseListener extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (e.getSource() == lblTitle) {
				// Wird der Tabtitel mit der mittleren Maustaste gedrückt wird
				// dieses ChatWindow geschlossen
				if (e.getModifiersEx() == 512) {
					GUI.getGUI().delChat(owner);
					// Wird der Tabtitel anders angeklickt wird der Tab
					// selektiert und eventuelles blinken ausgeschalten
				} else {
		...
		public void mouseEntered(MouseEvent e) {
			...
			if (e.getSource() == lblClose) {
				// Ist die Maus über den lblClose und der Chatpartner online
				// wird die Farbe orange
				if (owner.getOnlineState()) {
					tabCloseImgIcon.setImage(Help.getIcon("TabCloseOrange.png",
							10).getImage());
					// Ist der Chatpartner offline wird die Farbe schwarz
				} else {
					tabCloseImgIcon.setImage(Help.getIcon("TabCloseBlack.png",
							10).getImage());
				}
			} else {
				JLabel source = (JLabel) e.getSource();
				// Ist die Maus über dem Tabtitel und der Chatpartner online
				// wird der Titel orange
				if (owner.getOnlineState()) {
		...
		public void mouseExited(MouseEvent e) {
			// Eventquelle Schließenlabel[X] (lblClose)
			if (e.getSource() == lblClose) {
				// Verläßt die Maus den lblClose und der Chatpartner ist online
				// wird die Farbe orange
				if (owner.getOnlineState()) {
					tabCloseImgIcon.setImage(Help.getIcon("TabCloseBlack.png",
							10).getImage());
					...
					tabCloseImgIcon.setImage(Help.getIcon("TabCloseGray.png",
							10).getImage());
				}
			} else {
				JLabel source = (JLabel) e.getSource();
				// Verläßt die Maus den Tabtitel und der Chatpartner ist online
				// wird der Titel orange
				if (owner.getOnlineState()) {
					source.setForeground(Color.BLACK);
	...
	private class Blinker extends Thread {
		...
		public void run() {
			active = true;
			// lässt den Thread, solange aktiv, für die übergebene Zeit (delay)
			// schlafen und weckt ihn wieder
			while (active) {
				try {
					Thread.sleep(delay);
		 ...
		 * 
		 * Diese Methode setzt die Variable active auf false und deaktiviert
		 * somit den Thread
		 */
		private void stopit() {
			active = false;
