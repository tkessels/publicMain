 ...
 * 
 * Diese Klasse stellt die Kontaktliste zur Verfügung welche links am
 * Hauptfenster angedockt ist. Sie beinhaltet eine Liste mit allen Gruppen und
 * eine Liste mit allen erreichbaren Usern.
 * 
 * @author ATRM
 * 
 */
public class ContactList extends JWindow {
	...
	public ContactList(JFrame parent) {
		...
		this.usersScroller = new JScrollPane(users,
				JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		this.groupsScroller = new JScrollPane(groups,
				JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);

		// Panel für Gruppenliste und create Button um zusätzliche Gruppe zu
		// erstellen
		...
		// Layout Gruppenpanel
		this.groupPanel.setPreferredSize(new Dimension(breite, parent
				.getHeight() / 4));
		...
		this.createGrp.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				tmpGrpName = (String) JOptionPane.showInputDialog(GUI.getGUI(),
						"Enter Groupname", "Groupname",
						JOptionPane.OK_CANCEL_OPTION,
						Help.getIcon("gruppe.png"), null, null);
				if ((tmpGrpName != null)
						&& GUI.getGUI().checkName(tmpGrpName, 0)) {
					tmpGrpName = GUI.getGUI().confName(tmpGrpName, true);
					GUI.getGUI().addGrpCW(tmpGrpName, true);
				} else {
					GUI.getGUI()
							.info("Illegal charakter in groupname!<br>Allowed charakters: a-z,A-Z,0-9,ö,ä,ü,Ö,Ä,Ü,ß,é,á,-,_",
									null, 2);
				}
			}
		});
	 ...
	 * 
	 * Diese Methode überprüft die Gruppenliste ob eine Gruppe mit dem
	 * übergebenen String (name) bereits in der Gruppenliste enthalten ist und
	 * gibt dementsprechend true zurück. Ist eine Gruppe mit dem übergebenen
	 * String nicht enthalten wird false zurückgeliefert.
	 * 
	 * @param name
	...
	/**
	 * Diese Methode prüft ob ein User mit dem selben Alias in der Userliste
	 * existiert.
	 * 
	 * Diese Methode überprüft die Userliste ob ein User mit dem übergebenen
	 * String (alias) bereits in der Userliste enthalten ist und gibt
	 * dementsprechend true zurück. Ist ein User mit dem übergebenen String
	 * nicht enthalten wird false zurückgeliefert.
	 * 
	 * @param alias
	 *            String name des Users
	 * @return boolean true wenn existent, false falls nicht
	 */
	...
	public void repaint() {
		...
		setBounds((int) (tmp.getX() - breite), (int) tmp.getY(), breite,
				tmp.height);
		super.repaint();
	}// eom repaint()
	 ...
	 * 
	 * Diese Elementklasse stellt einen MouseAdapter zur Verfügung welcher auf
	 * Doppelklick bzw. auf rechte Maustaste reagiert.
	 * 
	 * @author ATRM
	 * 
	 */
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (e.getModifiersEx() == 256) {
				...
				if ((index >= 0) && (index < source.getModel().getSize())) {
					...
					// wenn das Element vom Typ Node ist wird ein Popup für die
					// Userlist geöffnet
					if (o instanceof Node) {
						Node tmp = (Node) o;
						PopupUser popupUsr = new PopupUser(new popupListener(
								tmp.getAlias()));
						popupUsr.show(source, e.getX(), e.getY());
					}
					// wenn in der Gruppenliste geklickt wurde wird ein Popup
					// für die Gruppenliste geöffnet
					if (source.getModel().getClass().getSimpleName()
							.startsWith("Group")) {
						PopupGroup popupGrp = new PopupGroup(o.toString(),
								new popupListener(o.toString()));
	 ...
	 * welches ein Kontextmenü darstellt in dem man einem User
	 * Flüsternachrichten und Dateien senden kann. In diesem Menü können User
	 * ignored/unignored werden sowie Informationen zu diesen ausgegeben werden.
	 * Man kann seinen eigenen Alias ändern und sich selbst als AFK markieren.
	 * 
	 * @author ATRM
	 * 
	 */
	private class PopupUser extends JPopupMenu {
		 ...
		 * 
		 * Dieser Konstruktor übernimmt einen ActionListener und erstellt ein
		 * PopupMenu für einen User.
		 * 
		 * @param popupListener
		 *            ActionListener
		 */
		private PopupUser(ActionListener popupListener) {
			...
			this.whisper = new JMenuItem("whisper to " + user, Help.getIcon(
					"whisperSym.png", 14, 16));
			this.sendFile = new JMenuItem("send File to " + user, Help.getIcon(
					"sendFileSym.png", 14, 16));
			this.ignore = new JMenuItem("ignore " + user, Help.getIcon(
					"ignoreSym.png", 12, 16));
			this.unignore = new JMenuItem("unignore " + user, Help.getIcon(
					"unignoreSym.png", 14, 16));
			this.info = new JMenuItem("info's about " + user,
					Help.getIcon("infoSym.png"));
			this.changeAlias = new JMenuItem("change Alias", Help.getIcon(
					"changeAliasSym.png", 14, 16));
			this.afkStatus = new JMenuItem("AFK (on/off)", Help.getIcon(
					"afkSym.png", 16, 12));
	 ...
	 * 
	 * Diese Elementklasse stellt das PopupMenu für die Gruppenliste zur
	 * Verfügung welches ein Kontextmenü darstellt in dem man einer Gruppe
	 * beitreten oder diese verlassen kann.
	 * 
	 * @author ATRM
	 * 
	 */
	private class PopupGroup extends JPopupMenu {
		...
		private PopupGroup(String group, ActionListener popupListener) {
			...
			this.join = new JMenuItem("join " + group,
					Help.getIcon("joinGrpSym.png"));
			this.leave = new JMenuItem("leave " + group,
					Help.getIcon("leaveGrpSym.png"));
	...
	private class popupListener implements ActionListener {
		 ...
		 * 
		 * Dieser Konstruktor übernimmt einen String (chatname) und setzt die
		 * Variable chatname auf den übergebenen String.
		 * 
		 * @param chatname
		 *            Strin
		 */
		...
		public void actionPerformed(ActionEvent e) {
			...
			// Klick auf "change Alias"
				...
				tmpAlias = (String) JOptionPane.showInputDialog(GUI.getGUI(),
						"Enter new Alias", "Change Alias",
						JOptionPane.OK_CANCEL_OPTION,
						Help.getIcon("private.png"), null, null);
	 ...
	 * 
	 * Diese Elementklasse ermöglicht es die Elemente der Userliste wie
	 * gewünscht darzustellen.
	 * 
	 * @author ATRM
	 * 
	 */
	private class UsersListCellRenderer extends JLabel implements
			ListCellRenderer {
		...
		public Component getListCellRendererComponent(JList paramlist,
			...
			setText(cutText[0]);
			Node tmp = (Node) value;
			// wenn ein Nutzer ignoriert wird soll er rot angezeigt werden und
			// einen
			// Zusatz zum Alias "(ignored)" erhalten
			if (ChatEngine.getCE().is_ignored(tmp.getNodeID())) {
				setText(cutText[0] + " (ignored)");
				setForeground(Color.RED);
				// Falls man AFK geht wird der eigene Name in der Liste grau und
				// mit einen
				// Zusatz "(afk)" versehen
	 ...
	 * 
	 * Diese Elementklasse ermöglicht es die Elemente der Gruppenliste wie
	 * gewünscht darzustellen.
	 * 
	 * @author ATRM
	 * 
	 */
	private class GroupsListCellRenderer extends JLabel implements
			ListCellRenderer {
