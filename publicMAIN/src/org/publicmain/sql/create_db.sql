...
CREATE  TABLE IF NOT EXISTS `db_publicmain`.`t_msgType` (  `name` ENUM('GROUP','PRIVATE','NODE_UPDATE','ALIAS_UPDATE','NODE_LOOKUP','ECHO_REQUEST','ECHO_RESPONSE','PATH_PING_REQUEST','PATH_PING_RESPONSE','ROOT_DISCOVERY','ROOT_REPLY','ROOT_ANNOUNCE','POLL_CHILDNODES','REPORT_CHILDNODES','POLL_ALLNODES','REPORT_ALLNODES','TREE_DATA_POLL','TREE_DATA','NODE_SHUTDOWN','CHILD_SHUTDOWN','GROUP_POLL','GROUP_REPLY','GROUP_JOIN','GROUP_LEAVE','GROUP_EMPTY','GROUP_ANNOUNCE','FILE_REQUEST','FILE_REPLY','FILE_RECIEVED','FILE_TCP_REQUEST','FILE_TCP_REPLY','FILE_TCP_ABORT','CMD_SHUTDOWN','CMD_RESTART','CMD_RECONNECT','CW_INFO_TEXT','CW_WARNING_TEXT','CW_ERROR_TEXT','CW_FILE_REQUEST','GUI_INFORM') NOT NULL ,  PRIMARY KEY (`name`) )ENGINE = InnoDB;
-- -----------------------------------------------------
-- Table `db_publicmain`.`t_messages`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `db_publicmain`.`t_messages` ;
CREATE  TABLE IF NOT EXISTS `db_publicmain`.`t_messages` (  `timestamp` BIGINT(20) NOT NULL ,  `msgID` INT(11) NOT NULL ,  `data` VARCHAR(200) NOT NULL ,  `t_user_userID_sender` BIGINT(20) NOT NULL ,  `t_msgType_name` ENUM('GROUP','PRIVATE','NODE_UPDATE','ALIAS_UPDATE','NODE_LOOKUP','ECHO_REQUEST','ECHO_RESPONSE','PATH_PING_REQUEST','PATH_PING_RESPONSE','ROOT_DISCOVERY','ROOT_REPLY','ROOT_ANNOUNCE','POLL_CHILDNODES','REPORT_CHILDNODES','POLL_ALLNODES','REPORT_ALLNODES','TREE_DATA_POLL','TREE_DATA','NODE_SHUTDOWN','CHILD_SHUTDOWN','GROUP_POLL','GROUP_REPLY','GROUP_JOIN','GROUP_LEAVE','GROUP_EMPTY','GROUP_ANNOUNCE','FILE_REQUEST','FILE_REPLY','FILE_RECIEVED','FILE_TCP_REQUEST','FILE_TCP_REPLY','FILE_TCP_ABORT','CMD_SHUTDOWN','CMD_RESTART','CMD_RECONNECT','CW_INFO_TEXT','CW_WARNING_TEXT','CW_ERROR_TEXT','CW_FILE_REQUEST','GUI_INFORM') NOT NULL ,  `t_user_userID_empfaenger` BIGINT(20) NOT NULL ,  `t_groups_name` VARCHAR(20) NOT NULL ,  PRIMARY KEY (`msgID`, `timestamp`, `t_user_userID_sender`) ,  CONSTRAINT `fk_t_messages_t_user1`    FOREIGN KEY (`t_user_userID_sender` )    REFERENCES `db_publicmain`.`t_user` (`userID` )    ON DELETE NO ACTION    ON UPDATE NO ACTION,  CONSTRAINT `fk_t_messages_t_msgType1`    FOREIGN KEY (`t_msgType_name` )    REFERENCES `db_publicmain`.`t_msgType` (`name` )    ON DELETE NO ACTION    ON UPDATE NO ACTION,  CONSTRAINT `fk_t_messages_t_user2`    FOREIGN KEY (`t_user_userID_empfaenger` )    REFERENCES `db_publicmain`.`t_user` (`userID` )    ON DELETE NO ACTION    ON UPDATE NO ACTION,  CONSTRAINT `fk_t_messages_t_groups1`    FOREIGN KEY (`t_groups_name` )    REFERENCES `db_publicmain`.`t_groups` (`name` )    ON DELETE NO ACTION    ON UPDATE NO ACTION)ENGINE = InnoDB;
